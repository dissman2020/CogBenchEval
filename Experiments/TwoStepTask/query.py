...
class TwoStepTaskExpForLLM(Experiment):
    ...
    def add_arguments_(self):
        ...
        self.parser.add_argument('--num_trials', type=int, default=20,
                                 help='Number of trials. Set to 20 by default to replicate (Binz & Schulz, 2023).')
    ...
    def run_single_experiment(self, llm):
        ...
        if self.parser.parse_args().version_number == '1':
            for i in range(self.num_trials):
                ...
                row = [i, action1, state, action2, treasure, reward_probs[0], reward_probs[1], reward_probs[2],
                       reward_probs[3]]
                data.append(row)
        ...
        df = pd.DataFrame(data, columns=['trial', 'action1', 'state', 'action2', 'reward', 'probsA', 'probsB', 'probsC',
                                         'probsD'])
        return df
